...
const TestResults = () => {
  ...
  const fetchTestResults = useCallback(async () => {
    try {
      const response = await axios.get(
        API_ENDPOINTS.STUDENT_TEST_RESULTS(testId)
      )
  ...
  const getAnswerForQuestion = questionId => {
    ...
    for (const sectionAttempt of attempt.sectionAttempts) {
      const answer = sectionAttempt.answers.find(
  ...
  const getPartialScore = (question, userAnswer) => {
    ...
    if (userAnswer.testCaseResults && userAnswer.testCaseResults.length > 0) {
      const passedCount = userAnswer.testCaseResults.filter(
        ...
        <div className='bg-white rounded-lg shadow-sm p-6 mb-8'>
          <div className='grid grid-cols-1 md:grid-cols-6 gap-4'>
            ...
            <div className='text-center'>
              ...
              <div className='text-2xl font-bold text-gray-900'>
                {earnedPoints.toFixed(1)}/{totalPoints}
              </div>
        ...
        <div className='bg-white rounded-lg shadow-sm p-6 mb-8'>
          ...
          <div className='space-y-4'>
            {test.sections.map((section, sectionIndex) => {
              ...
              const sectionTotalPoints = sectionQuestions.reduce(
                (total, question) => total + (question.points || 1),
                0
              )
              const sectionEarnedPoints = sectionQuestions.reduce(
                (total, question) => {
                  const userAnswer = getAnswerForQuestion(question._id)
                  if (isAnswerCorrect(question, userAnswer)) {
              ...
              const sectionPercentage =
                sectionTotalPoints > 0
                  ? Math.round((sectionEarnedPoints / sectionTotalPoints) * 100)
                  : 0
                ...
                >
                  <div className='flex-1'>
                    ...
                    <p className='text-sm text-gray-600'>
                      {sectionCorrect}/{sectionQuestions.length} questions
                      correct • {sectionEarnedPoints.toFixed(1)}/
                          ...
                          <div className='flex items-center space-x-2'>
                            ...
                            >
                              ...
                              userAnswer?.testCaseResults
                                ? (() => {
                                    const passedCount =
                                      userAnswer.testCaseResults.filter(
                                        r => r.passed
                                      ).length
                                    const totalCount =
                                      userAnswer.testCaseResults.length
                                    const partialScore = getPartialScore(
                                      question,
                                      userAnswer
                                    )
                                    const maxScore = question.points || 1
                                    return `${partialScore.toFixed(
                                      1
                                    )}/${maxScore} pts (${passedCount}/${totalCount} passed)`
                                  })()
                                : (() => {
                                    if (!userAnswer)
                                      return `0/${
                          ...
                          <div className='space-y-3'>
                            {question.options.map((option, optionIndex) => {
                              const isUserSelected =
                                userAnswer?.selectedOptions?.includes(
                                  optionIndex
                                )
                            ...
                            {userAnswer?.testCaseResults && (
                              <div>
                                ...
                                <div className='space-y-2'>
                                  {userAnswer.testCaseResults.map(
                                    (result, index) => (
                                      <div
                                        key={index}
