...
router.get('/tests', studentAuth, async (req, res) => {
  try {
    ...
    // For debugging: show all active tests (remove this restriction later)
    ...
    console.log(`Found ${tests.length} active tests for student ${req.user._id}`);
    ...
    res.json({ tests: testsWithAttempts });
  } catch (error) {
...
router.get('/tests/:id', studentAuth, async (req, res) => {
  try {
    ...
    // For debugging: temporarily allow all students to access tests
    // TODO: Re-enable this check once student assignment is working
    /*
